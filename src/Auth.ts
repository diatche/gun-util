...
export default class Auth {
  ...
  static default(gun?: IGunChainReference) {
    if (!this._default) {
      ...
      if (!isGunInstance(gun)) {
        throw new GunError(
          'Must specify a valid gun instance or set a default'
        );
      }
  ...
  async join(options: AuthBasicOptions = {}): Promise<void> {
    ...
    if (timeout) {
      await timeoutAfter(joins, timeout).catch(err => {
        stop = true;
        throw err;
      });
    } else {
  ...
  private async _login(
    creds:
      | (UserCredentials & Partial<IGunCryptoKeyPair>)
      | (Partial<UserCredentials> & IGunCryptoKeyPair),
    options: AuthBasicOptions
  ): Promise<string> {
    const {
  ...
  private async _recallSessionStorage(
    ...
    return Promise.race([recallAction, this.on()]);
  }
  ...
  private async _create(
    ...
    if (alias) {
      ...
      let exists = await this.exists({ alias }, { timeout: existsTimeout });
      if (exists) {
    ...
    let createAction = new Promise<string>((resolve, reject) => {
      ...
      if (this.pub()) {
        throw new MultipleAuthError(
          'Should not be logged in when creating a user'
        );
      }
